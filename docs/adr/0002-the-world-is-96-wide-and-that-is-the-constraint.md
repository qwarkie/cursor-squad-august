...
**A branch's reach is not what the constant says.** `STREAM_REACH` is 10 for every branch, and an unclamped branch gets it — measured as 10.5, or 10.0 where the trunk width is odd, which is the parity of the half-width subtraction and not a reduction. What takes water away is the clamp in `tributaryEnd`, and it is worth being exact about where it sits: the **village anchor** is clamped to `[ceil(VILLAGE_HALF_W), WORLD_W - ceil(VILLAGE_HALF_W)]` — `[14, 82]`, not `[0, 96]`. On the seeded month only Housing hits it: it wants an anchor at 86, is given 82, and loses exactly those 4 art-px of visible water (10.5 -> 6.5). Every other branch is free.

The causal path is one step longer than "a wide trunk leaves less room": `tributaryEnd` already **compensates** for trunk width by adding `bank` to the reach, so a wider trunk pushes its village further out. The compensation is correct and it is what drives the anchor into the clamp. **§5 does not need to add compensation — it needs somewhere for the existing compensation to go.** The reduction depends on the meander phase at that `atY` and on the trunk width there — which is set by the amounts listed *above* the branch. **It never depends on the branch's own amount.** In the seeded month the first category is also the largest, which makes this read as an inverted signal; it is not inverted, it is unrelated, and the correlation is an accident of the demo budget.
