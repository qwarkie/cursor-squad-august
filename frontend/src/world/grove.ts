...
/**
 * Foliage for a world of `worldH` art-pixels.
 *
 * Height is a parameter rather than an import so this module stays free of
 * `World.tsx` and therefore of React — it runs in Node, which is the whole
 * reason the placement is testable at all.
 */
export function grove(model: GroveInput, worldH: number): GroveSpot[] {
  const spots: GroveSpot[] = []

  for (let row = 0, y = STEP_Y; y < worldH; y += STEP_Y, row++) {
    for (let x = row % 2 === 0 ? 4 : 4 + ROW_STAGGER; x < WORLD_W; x += STEP_X) {
      ...
      const kind: Foliage = (row + Math.floor(x / STEP_X)) % 3 === 0 ? 'bush' : 'tree'
      const { w, h } = SIZE[kind]

      if (x < 1 || x + w > WORLD_W - 1) continue
      if (y < 1 || y + h > worldH - 1) continue
